...
_REQUIRED_FIELDS = {"source_uri", "title", "text"}
    ...
    async def ingest_json_dir(self, path: Path) -> list[dict[str, Any]]:
        ...
        for file_path in files:
            try:
                ...
                self._validate_payload(payload, file_path)
                ...
                logger.exception("ingestion_failed", extra={"file": str(file_path)})
                results.append({"file": str(file_path), "error": True})
        return results

    @staticmethod
    def _validate_payload(payload: dict[str, Any], file_path: Path) -> None:
        missing = _REQUIRED_FIELDS - set(payload.keys())
        if missing:
            raise ValueError(f"Missing required fields {missing} in {file_path.name}")
